namespace :load do
  task :defaults do
    ...
    set :postgres_remote_database_staging, ask('Remote Postgres Database:', 'sharengo_staging')
    set :postgres_remote_database_live, ask('Remote Postgres Database:', 'sharengo_live')
    set :postgres_remote_database_test, ask('Remote Postgres Test Database:', 'sharengo_test')
    ...
    on roles(fetch(:postgres_role)) do |role|
      ...
      begin
        ...
        unless fetch(:postgres_remote_locale_sql_file_path)
          file_name = "locale_backup.#{time}.sql"
          set :postgres_remote_locale_sql_file_path, "#{shared_path}/#{fetch(:postgres_locale_backup_dir)}/#{file_name}"
        end
        
        
        dump_locale_query = " pg_dump -U #{postgres_remote_username} " \
                            " -h #{postgres_remote_host} "\
                            " #{postgres_remote_database} " \
                            " -t locale_platforms  -t locale_sections -t locale_variables " \
                            " -c > #{fetch(:postgres_remote_locale_sql_file_path)} "
  ...
  task :import do
    run_locally do
      unless fetch(:database_name)
        ask(:database_name, 'sharengo_dev')
      end
